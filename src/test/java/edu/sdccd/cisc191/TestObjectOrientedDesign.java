...
class TestObjectOrientedDesign
{
    ...
    void testOption()
    {
        Option roof = new Option("Moon roof");
        assertEquals("Moon roof", roof.getDetails());
        Option leather = new Option("Leather");
        assertEquals("Leather", leather.getDetails());
        Option[] options = { roof, leather };
        Vehicle vehicle = new Vehicle("BMW", 90000, 10995, 4, options);

        assertEquals("Moon roof", vehicle.getOptions()[0].getDetails());
    ...
    void testHybridCarIsCombustibleAndChargeable()
    {
        ...
        assertEquals(100, ((Combustible) car).getFuelLevel());
        assertEquals(100, ((Chargeable) car).getBatteryCharge());
    }
